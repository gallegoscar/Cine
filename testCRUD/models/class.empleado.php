...
function delete($id){
	$sql = "DELETE FROM administrador.tbl_empleadoWHERE id = '$id'";
	try {
		pg::query("begin");
		$row = pg::query($sql);
		pg::query("commit");
...
//echo "me llamo";
	if ($this->validaP($this->id) == false){
		...
		$sql="UPDATE administrador.tbl_empleado set nombre='" . $this->nombre . "' WHERE id='" . $this->id . "'WHERE salario='".$this->salario."'";
		pg::query("begin");
		$row = pg::query($sql);
		pg::query("commit");		
		echo "2";
